...
import { rooms, history7d, devicesMock, solutions, recentEvents, lightingCurve, forecastData } from "@/lib/mockData";
import {
  Thermometer, Droplets, FlaskConical, Zap, Wind, Sparkles, AlertTriangle,
  Wifi, WifiOff, Battery, Sun, Flower2, Clock, Droplet, Leaf, TrendingUp, Brain,
} from "lucide-react";
...
const tabs = ["Overview", "Flowering", "Solutions", "Setpoints", "Forecast", "History", "Devices"] as const;
...
const eventIconMap: Record<string, typeof Sun> = {
  dose: FlaskConical, irrigation: Droplet, alert: AlertTriangle, light: Sun,
};
const eventColorMap: Record<string, string> = {
  dose: "rgba(46,168,74,0.14)", irrigation: "rgba(95,212,126,0.10)",
  alert: "rgba(255,209,102,0.14)", light: "rgba(255,209,102,0.10)",
};
const eventTextMap: Record<string, string> = {
  dose: "#2EA84A", irrigation: "#5fd47e", alert: "#FFD166", light: "#FFD166",
};
...
function FloweringTab({ room }: { room: typeof rooms[number] }) {
  ...
  const stageColor = room.stage === "Flowering" ? "#FFD166" : room.stage === "Vegetative" ? "#5fd47e" : "#8ab894";
    ...
    <section className="px-5 mt-2 space-y-3">
      <div className="glass rounded-3xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full" style={{ background: `${stageColor}22`, color: stageColor }}>
            {weeksLeft}w left
          </span>
        </div>
      ...
      <div className="glass rounded-2xl p-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm font-semibold text-ink mt-0.5">
            {room.stage === "Flowering" ? "Flush week" : room.stage === "Vegetative" ? "Switch to Flowering" : "Transplant ready"}
          </p>
      ...
      <div className="space-y-2">
        {recentEvents.map((e, i) => {
          ...
          return (
            <motion.div key={e.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }} className="glass rounded-2xl p-3 flex items-center gap-3">
...
function SolutionsTab() {
  return (
    <section className="px-5 mt-2 space-y-3">
      ...
      {solutions.map((s, i) => (
        <motion.div key={s.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} className="glass rounded-2xl p-4 flex items-start gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "rgba(255,209,102,0.12)" }}>
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-white/5 last:border-0">
      ...
      <div className="flex items-center gap-3">
        ...
        >−</button>
        <span className="text-sm font-num font-semibold text-ink w-14 text-center">{fmt(value)}</span>
        <button
...
function RangeField({
  label, unit, value, hardMin, hardMax, step = 0.5,
}: {
    ...
    <div className="rounded-xl p-3.5" style={{ background: "rgba(255,255,255,0.03)" }}>
      ...
      <div className="relative h-1.5 rounded-full mb-3" style={{ background: "rgba(255,255,255,0.06)" }}>
        <div
          ...
          style={{ left: `${lowPct}%`, width: `${highPct - lowPct}%`, background: "linear-gradient(90deg, #2EA84A, #5fd47e)", boxShadow: "0 0 10px rgba(46,168,74,0.45)" }}
        />
      </div>
      <Stepper label="Min" value={v[0]} min={hardMin} max={v[1] - step} step={step} format={fmt}
        onChange={(n) => setV([n, v[1]])} />
      <Stepper label="Max" value={v[1]} min={v[0] + step} max={hardMax} step={step} format={fmt}
    ...
    <section className="px-5 mt-2 space-y-4">
      ...
      >
        ...
        <div className="flex-1 relative">
          <p className="text-sm font-semibold text-ink">{aiApplied ? "AI Recommendations Applied" : "Apply AI Recommendations"}</p>
          <p className="text-[11px] text-ink-dim mt-0.5">{aiApplied ? "All setpoints optimised for this room" : `Optimised for ${room.stage} · ${room.name}`}</p>
        </div>
        {!aiApplied && (
          <span className="relative text-xs font-semibold px-3 py-1.5 rounded-full text-[#06120a]" style={{ background: "linear-gradient(135deg, #2EA84A, #5fd47e)" }}>Apply</span>
        )}
      </motion.button>
      ...
      <div className="glass rounded-2xl p-4 space-y-3">
        ...
        <svg viewBox="0 0 240 60" className="w-full h-12" preserveAspectRatio="none">
          ...
          <polyline
            ...
            fill="none" stroke="#FFD166" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
            style={{ filter: "drop-shadow(0 0 4px rgba(255,209,102,0.6))" }}
          />
          <polygon
            points={["0,60", ...lightingCurve.map((p) => `${(p.h / 24) * 240},${60 - (p.v / 100) * 50}`), "240,60"].join(" ")}
            fill="url(#lc-grad)"
          />
        </svg>
        ...
        <div className="rounded-xl p-3" style={{ background: "rgba(255,255,255,0.03)" }}>
          <Stepper label="Lights on" value={lightOn} min={0} max={lightOff - 1} step={1}
            format={(v) => `${String(v).padStart(2, "0")}:00`} onChange={setLightOn} />
          <Stepper label="Lights off" value={lightOff} min={lightOn + 1} max={24} step={1}
            format={(v) => `${String(v).padStart(2, "0")}:00`} onChange={setLightOff} />
        </div>
      </div>
      ...
      <div className="glass rounded-2xl p-4 space-y-3">
        ...
        <RangeField label="Temperature" unit="°C" value={t.temp} hardMin={15} hardMax={35} step={0.5} />
        <RangeField label="Humidity" unit="%" value={t.humidity} hardMin={30} hardMax={90} step={1} />
      ...
      <div className="glass rounded-2xl p-4">
        ...
        <div className="rounded-xl p-3" style={{ background: "rgba(255,255,255,0.03)" }}>
          <Stepper label="Cycle interval" value={irrigInterval} min={1} max={12} step={1}
            format={(v) => `every ${v}h`} onChange={setIrrigInterval} />
          <Stepper label="Duration" value={irrigDuration} min={1} max={15} step={1}
            format={(v) => `${v} min`} onChange={setIrrigDuration} />
        </div>
      </div>
...
function ForecastChart({
  history, forecast, target, color, unit,
}: {
  history: number[]; forecast: number[]; target: [number, number]; color: string; unit: string;
}) {
  ...
  return (
    <div className="w-full overflow-hidden">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 100 }} preserveAspectRatio="none">
        ...
        <line x1="0" y1={targetY1} x2={W} y2={targetY1} stroke={color} strokeWidth="0.5" strokeDasharray="3 3" opacity="0.4" />
        <line x1="0" y1={targetY0} x2={W} y2={targetY0} stroke={color} strokeWidth="0.5" strokeDasharray="3 3" opacity="0.4" />

        {/* Divider history/forecast */}
        <line x1={joinX} y1="0" x2={joinX} y2={H} stroke="rgba(255,255,255,0.12)" strokeWidth="1" strokeDasharray="3 3" />

        {/* History fill */}
        <polygon
          points={`0,${H} ${histPts} ${joinX},${H}`}
          fill={`url(#hg-${color})`}
        />
        {/* History line */}
        <polyline
        ...
        {forecast.map((v, i) => {
          ...
          return (
            <circle key={i} cx={cx} cy={cy} r="3"
              ...
              stroke="rgba(10,26,15,0.8)" strokeWidth="1.5"
            />
          );
        })}

        {/* Current value dot */}
        <circle cx={joinX} cy={joinY} r="3.5" fill={color} stroke="rgba(10,26,15,0.8)" strokeWidth="1.5" />
      </svg>

      {/* x-axis labels */}
      <div className="flex justify-between text-[9px] font-num text-ink-dim mt-1 px-0.5">
        {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun", "+1", "+2", "+3", "+4", "+5"].slice(0, totalPts).map((d, i) => (
          <span key={i} className={i >= history.length ? "text-ink-soft/60" : ""}>{d}</span>
        ))}
      </div>
    </div>
  );
  ...
  return (
    <section className="px-5 mt-2 space-y-4">
      ...
      <div className="glass rounded-2xl p-4 flex items-center gap-3 relative overflow-hidden">
        <div className="absolute -right-6 -top-6 w-28 h-28 rounded-full" style={{ background: "radial-gradient(circle, rgba(95,212,126,0.18), transparent 70%)" }} />
        <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "rgba(46,168,74,0.12)" }}>
          <Brain className="w-5 h-5 text-primary" />
        </div>
        <div className="relative">
          <p className="text-sm font-semibold text-ink">AI Forecast · LSTM Model</p>
          <p className="text-[11px] text-ink-dim">7-day history + 5-day prediction for {room.name}</p>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {allAlerts.map((a, i) => (
            <motion.div
              ...
              style={{ background: "rgba(255,209,102,0.10)", border: "1px solid rgba(255,209,102,0.2)" }}
            >
              <AlertTriangle className="w-4 h-4 text-warning flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-warning">{a.metric} alert in +{a.day}d</p>
      ...
      {metrics.map((m, i) => {
        ...
        const outOfRangeDays = fd.forecast.filter((v) => v < fd.target[0] || v > fd.target[1]).length;
          ...
          >
                ...
                <div className="flex items-center gap-1">
                  <TrendingUp
                    ...
                    style={{
                      ...
                      transform: trend === "down" ? "scaleY(-1)" : trend === "stable" ? "rotate(0deg)" : undefined,
                    }}
                  />
            ...
            <div className="flex gap-1.5 mt-3 overflow-x-auto scrollbar-hide">
              {fd.forecast.map((v, idx) => {
                ...
                return (
                  <div key={idx} className="flex-shrink-0 rounded-lg px-2.5 py-1.5 text-center" style={{ background: bad ? "rgba(255,107,107,0.12)" : "rgba(255,255,255,0.04)", border: `1px solid ${bad ? "rgba(255,107,107,0.3)" : "rgba(255,255,255,0.06)"}` }}>
                    <p className="text-[9px] text-ink-dim">+{idx + 1}d</p>
                    <p className="text-[11px] font-num font-semibold mt-0.5" style={{ color: bad ? "#FF6B6B" : m.color }}>
                      {v}{fd.unit}
                    </p>
                  </div>
                );
  ...
  return (
    <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-24">
      ...
      <polyline
        ...
        className="draw-line" style={{ filter: `drop-shadow(0 0 4px ${color}80)` }}
      />
    </svg>
  );
}
...
function HistoryTab() {
  ...
  return (
    <section className="px-5 mt-2 space-y-3">
      ...
      {charts.map((c, i) => (
        <motion.div key={c.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} className="glass rounded-2xl p-4">
...
function DevicesTab({ roomName }: { roomName: string }) {
  return (
    <section className="px-5 mt-2 space-y-2.5">
      ...
      {devicesMock.map((d, i) => (
        <motion.div key={d.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="glass rounded-2xl p-4 flex items-center gap-3">
